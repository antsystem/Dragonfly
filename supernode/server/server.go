...
func New(cfg *config.Config, logger *logrus.Logger, register prometheus.Registerer) (*Server, error) {
	...
	rtTaskMgr, err := realtime_task.NewManager(cfg)
	if err != nil {
		return nil, err
	}

	gcMgr, err := gc.NewManager(cfg, taskMgr, peerMgr, dfgetTaskMgr, progressMgr, cdnMgr, rtTaskMgr, register)
	if err != nil {
		return nil, err
	}
