...
func (sd *seed) initParam(metaDir string) error {
	...
	sd.blockMeta, err = bitmap.NewBitMap(uint32(sizeOf64Bits), false)
	if err != nil {
		return err
	}

	sd.lockBlock, err = bitmap.NewBitMap(uint32(sizeOf64Bits), false)
	if err != nil {
		return err
	}
...
func (sd *seed) tryDownload(startBlock, endBlock uint32, rateLimit bool) (waitChs []chan struct{}) {
	rs, err := sd.blockMeta.Get(uint32(startBlock), uint32(endBlock), false)
...
func (sd *seed) downloadBlock(blockStartIndex, blockEndIndex uint32, rateLimit bool) {
	...
	sd.blockMeta.Set(uint32(blockStartIndex), uint32(blockEndIndex), true)
}
...
func (sd *seed) prefetchAllBlocks(ctx context.Context, perDownloadBlocks int32) error {
	...
	for {
		if blockIndex >= uint32(sd.blocks) {
			break
		}
