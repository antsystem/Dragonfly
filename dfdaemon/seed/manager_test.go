...
func (suite *SeedTestSuite) TestManySeed(c *check.C) {
	...
	for i := 0; i < 4; i++ {
		...
		sd, err := sm.Register(taskIDArr[i], PreFetchInfo{URL: fmt.Sprintf("http://%s/%s", suite.host, filePaths[i]),
...
func (suite *SeedTestSuite) TestSeedSyncWriteAndRead(c *check.C) {
	...
	sd, err := sm.Register(taskIDArr[0], PreFetchInfo{URL: fmt.Sprintf("http://%s/%s", suite.host, filePaths[0]),
